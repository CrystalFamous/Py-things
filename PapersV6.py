...
def routines():
    prompts = ["Enter year: ", "Which month? (s,w,m): ", "Which Paper: ", "in or qp or ms or both or all(insert sheet) or csP4(qp/ms/sf): "]
    inputs = []
...
def subject(subject_code: str, subject_index: int, year_range: str):
    ...
    default = Paper[subject_index] + year + "/" + subject_code + "_" + month + mid + "_qp_" + paper + ".pdf"
    if choice in ["qp", "1", "ms", "2", "in", "3"]:
...
def extractv2(sub:str):
    ...
    if sub [:4] in paper_dict:
        print("Input in qp format #9618_w21_qp_42")
        choice = input("Do you want to open both(qp/ms) or all(in)? or CsP4(sf) /N?: ").lower()
        if choice == "n":
            if "qp" in sub:
            ...
            sub = sub.replace("ms","sf")
            Link3 = Paper[paper_dict[sub[:4]]] + Y + "/" + sub + ".zip"
            openlinks(Link,Link2,Link3)
        else:
            Link = Paper[paper_dict[sub[:4]]] + Y + "/" + sub + ".pdf"
            sub = sub.replace("qp","ms")
...
def main():
    ...
    if "_" in sub:
        ...
        eco()
    elif sub in ["cs", "4", "computerscience"]:
        cs()
